...
class TestRegisteredModelsSatisfyTheContract:
    ...
    def _macos_entries():
        ...
        # Community hybrid ports describe their export unit through
        # ``build_macos_export_spec`` / ``export_core()`` (consumed by
        # ``export_macos_model``) and need their config shims to construct, so
        # the vanilla-config contract sweep below does not apply to them.
        def _uses_spec_hook(cls) -> bool:
            return hasattr(cls, "build_macos_export_spec") or hasattr(cls, "export_core")

        return sorted(
            (
                (mt, e)
                for mt, e in _get_registry().items()
                if e.macos_class is not None and not _uses_spec_hook(e.macos_class)
            ),
            key=lambda kv: kv[0],
        )
